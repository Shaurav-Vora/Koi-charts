...
describe("connection labels",()=>{
 ...
 const before=structuredClone(state.graph.nodes);run({kind:"label_edge",target:{kind:"edge_id",id:"3"},label:"Yes"});expect(state.graph.edges).toEqual([{id:"3",source:"1",target:"2",label:"Yes"}]);expect(state.graph.nodes).toEqual(before);
 run({kind:"undo"});expect(state.graph.edges[0].label).toBeUndefined();run({kind:"redo"});expect(state.graph.edges[0].label).toBe("Yes");run({kind:"label_edge",target:{kind:"edge_id",id:"3"},label:null});expect(state.graph.edges[0].label).toBeUndefined();
 });
 it("rejects missing arrows and agrees in both command validators",()=>{const command={kind:"label_edge",target:{kind:"edge_id",id:"missing"},label:"No"};expect(commandSchema.safeParse(command).success).toBe(true);expect(new Ajv({strict:false}).compile(commandJsonSchema)({command})).toBe(true);expect(execute(createEngineState(),command,()=>"id").outcome).toBe("error");});
});
