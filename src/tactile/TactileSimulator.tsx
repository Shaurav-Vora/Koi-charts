...
function PinSurface({frame:incoming}:{frame:TactileFrame}) {
  ...
  return <svg className="tactile-pins" viewBox="0 0 120 80" role="img" aria-label={`${frame.mode} tactile view: ${frame.nodeIds.length} nodes, ${frame.edgeIds.length} connections. Raised cross marks focus.`}><rect width="120" height="80" fill="#edf1f8"/>{frame.raisedPins.map(pin=><circle key={pin.y*120+pin.x} cx={pin.x+.5} cy={pin.y+.5} r="0.42" fill="#17243a"/>)}</svg>;
}
