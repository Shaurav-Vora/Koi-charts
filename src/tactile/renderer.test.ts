...
describe("tactile renderer",()=>{
 ...
 it("includes labelled incoming and outgoing arrows in the focused Braille information",()=>{const labelled:FlowGraph={...graph,edges:[{...graph.edges[0],label:"next"},{...graph.edges[1],label:"yes"},graph.edges[2]]};const frame=makeTactileFrame(labelled,layoutGraph(labelled),"b","overview",1);const expected="Check process Incoming next from Begin Outgoing yes to Approved";expect(frame.text).toBe(expected);expect(frame.brailleCells).toBe(toBraille(expected).cells);});
