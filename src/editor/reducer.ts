...
import { createEngineState, execute, executeNodeSelectionDeletion, resolveClarification } from "../commands/execute";
...
export type EditorAction = { type: "example" } | { type: "clear" } | { type: "command"; command: GraphCommand; idSeed: string } | { type: "delete_selection"; nodeIds: string[]; idSeed: string } | { type: "choose"; candidateId: string; idSeed: string } | { type: "interface_focus"; focusedNodeId: string | null; message: string };
...
export function editorReducer(state: EditorState, action: EditorAction): EditorState {
  ...
  const result = action.type === "command"
