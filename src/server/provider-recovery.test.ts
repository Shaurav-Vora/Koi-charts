...
const completion = (content: unknown) => Response.json({ candidates: [{ finishReason: "STOP", content: { parts: [{ text: JSON.stringify(content) }] } }] });
...
describe("provider recovery", () => {
  it("preserves a provider 429 and waits out its cooldown without repeat calls", async () => {
    ...
    const routes = createRoutes({ transport, env: () => ({ GEMINI_API_KEY: "secret" }) });
  ...
  it.each([null, "invalid", "-1"])("uses a 60-second cooldown for unusable Retry-After %s", async header => {
    ...
    const routes = createRoutes({ transport: vi.fn().mockResolvedValue(response), env: () => ({ GEMINI_API_KEY: "secret" }) });
  ...
  it("honors an HTTP-date Retry-After", async () => {
    ...
    const routes = createRoutes({ transport: vi.fn().mockResolvedValue(new Response("rate limit", { status: 429, headers: { "Retry-After": "Sun, 06 Sep 2026 10:00:20 GMT" } })), env: () => ({ GEMINI_API_KEY: "secret" }) });
  ...
  it("distinguishes an explicit billing rejection from temporary throttling", async () => {
    ...
    const response = await createRoutes({ transport, env: () => ({ GEMINI_API_KEY: "secret" }) }).interpret(request());
  ...
  it("wraps a fully validated bare command", async () => {
    const transport = vi.fn().mockResolvedValueOnce(completion(command));
    const response = await createRoutes({ transport, env: () => ({ GEMINI_API_KEY: "secret" }) }).interpret(request());
  ...
  it.each([{ kind: "describe", scope: "wrong" }, { kind: "describe", scope: "chart", extra: true }])("still rejects invalid bare output %j", async command => {
    const transport = vi.fn().mockResolvedValueOnce(completion(command));
    expect((await createRoutes({ transport, env: () => ({ GEMINI_API_KEY: "secret" }) }).interpret(request())).status).toBe(502);
  });
});
